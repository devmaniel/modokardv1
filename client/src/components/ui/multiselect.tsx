    ...
    >
          ...
          {selected.map((option) => {
            return (
              <div
                key={option.value}
                className={cn(
                  "animate-fadeIn bg-primary border-primary cursor-pointer text-white relative inline-flex h-7  items-center rounded-md border ps-2 pe-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pe-2",
                  badgeClassName
                )}
              ...
              >
                {option.label}
                <button
                  className="text-white  focus-visible:border-ring focus-visible:ring-ring/50 absolute -inset-y-px -end-px flex size-7 items-center justify-center rounded-e-md border border-transparent p-0 outline-hidden transition-[color,box-shadow] outline-none focus-visible:ring-[3px]"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
      ...
      <div className="relative">
        ...
        >
          {open && (
            <CommandList
              className="bg- text-popover-foreground border shadow-lg outline-hidden"
              onMouseLeave={() => {
